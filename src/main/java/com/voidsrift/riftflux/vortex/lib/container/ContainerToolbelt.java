...
public class ContainerToolbelt extends Container {
   private final InventoryToolbelt inventoryToolbelt;
   private static final int invStart = 5;
   private static final int invEnd = 31;
   private static final int hotbarStart = 32;
   private static final int hotbarEnd = 40;

   public ContainerToolbelt(EntityPlayer player) {
      this.inventoryToolbelt = new InventoryToolbelt(player.getHeldItem());
      int xoff = 45;

      int i;
      for(i = 0; i < 5; ++i) {
         this.addSlotToContainer(new Slot(this.inventoryToolbelt, i, xoff + i * 18 - 1, 18) {
            public boolean isItemValid(ItemStack stack) {
               return ContainerHelper.toolbeltValid(stack);
            }
         });
      }
   ...
   public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int index) {
      ...
      if (slot != null && slot.getHasStack()) {
         ...
         } else if (index >= 5 && !this.mergeItemStack(itemstack1, 0, 5, false)) {
            return null;
         }
               ...
               if (!slot.isItemValid(stack)) {
                  ...
                  if (itemstack1 != null && itemstack1.getItem() == stack.getItem() && (!stack.getHasSubtypes() || stack.getItemDamage() == itemstack1.getItemDamage()) && ItemStack.areItemStackTagsEqual(stack, itemstack1)) {
                     ...
                     if (l <= stack.getMaxStackSize() && l <= slot.getSlotStackLimit()) {
                        ...
                        this.inventoryToolbelt.markDirty();
                        flag1 = true;
   ...
   public InventoryToolbelt getToolbelt() {
      return this.inventoryToolbelt;
   }
}
