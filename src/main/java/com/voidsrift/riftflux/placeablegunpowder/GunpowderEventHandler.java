...
public class GunpowderEventHandler {
    ...
    @SubscribeEvent
    public void placeGunpowder(PlayerInteractEvent event) {
        if (event.action != PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK
                || event.entityPlayer.getHeldItem() == null
                || event.entityPlayer.getHeldItem().getItem() != Items.gunpowder) {
            return;
        }
        ...
        if (BlockGunpowder.instance.canPlaceBlockAt(event.world, posX, posY, posZ)) {
            if (!event.entityPlayer.capabilities.isCreativeMode) {
                --event.entityPlayer.getHeldItem().stackSize;
            }
            event.world.setBlock(posX, posY, posZ, BlockGunpowder.instance);
        }
    }
}
