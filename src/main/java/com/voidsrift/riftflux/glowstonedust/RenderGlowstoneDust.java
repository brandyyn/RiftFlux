...
public class RenderGlowstoneDust implements ISimpleBlockRenderingHandler {
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
        if (modelId != RFRenderIds.glowstoneDustRenderId) {
            return false;
        }

        Tessellator tessellator = Tessellator.instance;
        int meta = world.getBlockMetadata(x, y, z);
        IIcon cross = BlockGlowstoneDust.getGlowstoneDustIcon("cross");
        IIcon line = BlockGlowstoneDust.getGlowstoneDustIcon("line");
        IIcon crossOverlay = BlockGlowstoneDust.getGlowstoneDustIcon("cross_overlay");
        IIcon lineOverlay = BlockGlowstoneDust.getGlowstoneDustIcon("line_overlay");
        ...
        if (meta > 0) {
            ...
            red = litAmount * 0.7F + 0.3F;
            green = litAmount * litAmount * 0.4F + 0.3F;
            blue = 0.3F;
            if (green < 0.0F) {
                green = 0.0F;
            }
        ...
        boolean west = BlockGlowstoneDust.canConnect(world, x - 1, y, z, 1)
                || !world.getBlock(x - 1, y, z).isBlockNormalCube()
                && BlockGlowstoneDust.canConnect(world, x - 1, y - 1, z, -1);
        boolean east = BlockGlowstoneDust.canConnect(world, x + 1, y, z, 3)
                || !world.getBlock(x + 1, y, z).isBlockNormalCube()
                && BlockGlowstoneDust.canConnect(world, x + 1, y - 1, z, -1);
        boolean north = BlockGlowstoneDust.canConnect(world, x, y, z - 1, 2)
                || !world.getBlock(x, y, z - 1).isBlockNormalCube()
                && BlockGlowstoneDust.canConnect(world, x, y - 1, z - 1, -1);
        boolean south = BlockGlowstoneDust.canConnect(world, x, y, z + 1, 0)
                || !world.getBlock(x, y, z + 1).isBlockNormalCube()
                && BlockGlowstoneDust.canConnect(world, x, y - 1, z + 1, -1);

        if (!world.getBlock(x, y + 1, z).isBlockNormalCube()) {
            if (world.getBlock(x - 1, y, z).isBlockNormalCube() && BlockGlowstoneDust.canConnect(world, x - 1, y + 1, z, -1)) {
                west = true;
            }
            if (world.getBlock(x + 1, y, z).isBlockNormalCube() && BlockGlowstoneDust.canConnect(world, x + 1, y + 1, z, -1)) {
                east = true;
            }
            if (world.getBlock(x, y, z - 1).isBlockNormalCube() && BlockGlowstoneDust.canConnect(world, x, y + 1, z - 1, -1)) {
                north = true;
            }
            if (world.getBlock(x, y, z + 1).isBlockNormalCube() && BlockGlowstoneDust.canConnect(world, x, y + 1, z + 1, -1)) {
                south = true;
        ...
        if (!world.getBlock(x, y + 1, z).isBlockNormalCube()) {
            if (world.getBlock(x - 1, y, z).isBlockNormalCube() && world.getBlock(x - 1, y + 1, z) == BlockGlowstoneDust.instance) {
                tessellator.setColorOpaque_F(red, green, blue);
            ...
            if (world.getBlock(x, y, z - 1).isBlockNormalCube() && world.getBlock(x, y + 1, z - 1) == BlockGlowstoneDust.instance) {
                tessellator.setColorOpaque_F(red, green, blue);
