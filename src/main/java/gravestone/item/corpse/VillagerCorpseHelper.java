...
public class VillagerCorpseHelper extends CorpseHelper {
   ...
   public static List<ItemStack> getDefaultCorpses(Item item, int corpseType) {
      ...
      list.add(getDefaultVillagerCorpse(item, corpseType, 0));
      list.add(getDefaultVillagerCorpse(item, corpseType, 1));
      list.add(getDefaultVillagerCorpse(item, corpseType, 2));
      list.add(getDefaultVillagerCorpse(item, corpseType, 3));
      list.add(getDefaultVillagerCorpse(item, corpseType, 4));
      ...
      if (recipes != null) {
         ...
         for(Object recipeObj : recipes) {
            ...
            if (recipe != null && recipe.getItemToBuy() != null && recipe.getItemToSell() != null) {
               NBTTagCompound recipeTag = recipe.writeToTags();
   ...
   private static String getType(NBTTagCompound nbtTag) {
      return ModGraveStone.proxy.getLocalizedString("item.corpse.villager_type") + " " + ModGraveStone.proxy.getLocalizedString(getVillagerProfession(getVillagerType(nbtTag)));
   }
   ...
   private static String getNotVanillaVillagerProfession(int type) {
      if (type == GraveStoneConfig.undertakerId) {
         ...
         return type == 206 ? "item.corpse.villager_type.thaumaturge" : "item.corpse.villager_type.unknown";
      }
   }
