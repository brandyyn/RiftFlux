...
public class HorseCorpseHelper extends CorpseHelper {
   ...
   public static List<ItemStack> getDefaultCorpses(Item item, int corpseType) {
      ...
      list.add(getDefaultHorseCorpse(item, corpseType, 0));
      list.add(getDefaultHorseCorpse(item, corpseType, 1));
      list.add(getDefaultHorseCorpse(item, corpseType, 2));
      list.add(getDefaultHorseCorpse(item, corpseType, 3));
      list.add(getDefaultHorseCorpse(item, corpseType, 4));
      return list;
   }
   ...
   private static ItemStack getDefaultHorseCorpse(Item item, int corpseType, int type) {
      ...
      nbtTag.setDouble("Max Health", 25.0D);
      nbtTag.setDouble("Movement Speed", 0.3D);
      nbtTag.setDouble("Jump Strength", 0.7D);
   ...
   private static String getType(NBTTagCompound nbtTag) {
      return ModGraveStone.proxy.getLocalizedString("item.corpse.horse_type") + " " + ModGraveStone.proxy.getLocalizedString(getHorseType(nbtTag.getInteger("HorseType")));
   }
