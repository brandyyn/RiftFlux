...
public class CatCorpseHelper extends CorpseHelper {
   ...
   public static List<ItemStack> getDefaultCorpses(Item item, int corpseType) {
      ...
      list.add(getDefaultCatCorpse(item, corpseType, 0));
      list.add(getDefaultCatCorpse(item, corpseType, 1));
      list.add(getDefaultCatCorpse(item, corpseType, 2));
      list.add(getDefaultCatCorpse(item, corpseType, 3));
      return list;
   }
   ...
   private static String getType(NBTTagCompound nbtTag) {
      return ModGraveStone.proxy.getLocalizedString("item.corpse.cat_type") + " " + ModGraveStone.proxy.getLocalizedString(getCatType(nbtTag.getByte("CatType")));
   }
