...
extends WEItemRanged {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (player.capabilities.isCreativeMode || player.inventory.hasItem((Item)WEArcheryItems.dart)) {
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World world, EntityPlayer player, int useCount) {
        ...
        for (int i = 0; i < this.getAllowedAmmo().size(); ++i) {
            if (!player.inventory.hasItem(this.getAllowedAmmo().get(i))) continue;
            hasAmmo = true;
        }
        if (flag || hasAmmo) {
            ...
            EntityDart dart = new EntityDart(world, (EntityLivingBase)player, f * 1.7f);
            if (ammo.hasTagCompound() && ammo.getTagCompound().hasKey(Dart.KEY_EFFECTID)) {
            ...
            if (!flag && player.inventory.hasItem(ammo.getItem())) {
