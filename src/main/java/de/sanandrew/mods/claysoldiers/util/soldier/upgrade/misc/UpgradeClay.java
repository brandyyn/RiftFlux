    ...
    public boolean onUpdate(EntityClayMan clayMan, SoldierUpgradeInst upgradeInst) {
        ...
        if (!clayMan.hasPath()) {
            ...
            for (EntityItem item2 : items) {
                if (item2.getEntityItem() == null || item2.getEntityItem().getItem() != RegistryItems.dollSoldier || !ItemClayManDoll.getTeam(item2.getEntityItem()).getTeamName().equals(clayMan.getClayTeam())) continue;
                clayMan.setTargetFollowing(item2);
                break;
            }
        } else if (clayMan.getTargetFollowing() instanceof EntityItem && (item = (EntityItem)clayMan.getTargetFollowing()).getEntityItem() != null && item.getEntityItem().getItem() == RegistryItems.dollSoldier && ItemClayManDoll.getTeam(item.getEntityItem()).getTeamName().equals(clayMan.getClayTeam()) && item.getDistanceSqToEntity(clayMan) < 1.0 && item.getEntityItem().stackSize > 0) {
