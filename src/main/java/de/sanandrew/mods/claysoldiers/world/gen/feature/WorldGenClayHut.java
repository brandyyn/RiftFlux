...
extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        Triplet chosenTeam = (Triplet)POSSIBLE_TEAMS.get(rand.nextInt(POSSIBLE_TEAMS.size()));
        WorldGenClayHut.genFloorAndRoof(world, x, y, z, (Integer)chosenTeam.getValue1());
        WorldGenClayHut.genWalls(world, x, y, z, (Integer)chosenTeam.getValue1());
        WorldGenClayHut.carveRoom(world, x, y, z);
        WorldGenClayHut.setDoorAndInteriors(world, rand, x, y, z, (Integer)chosenTeam.getValue1(), (ItemStack)chosenTeam.getValue2());
        WorldGenClayHut.placeSoldiers(world, rand, x, y, z, (String)chosenTeam.getValue0());
        return true;
    }
    ...
    public static void generate(World world, Random rand, int chunkX, int chunkZ) {
        ...
        int y = WorldGenClayHut.getSuitableY(world, x, z = (chunkZ << 4) + rand.nextInt(16));
        if (y > 0) {
            FMLLog.info((String)"Structure WorldGenClayHut: %d | %d | %d", (Object[])new Object[]{x, y, z});
    ...
    private static int getSuitableY(World world, int x, int z) {
        ...
        for (int i = 196; i >= 48; --i) {
            ...
            if (block == null || block != genBiome.topBlock) continue;
            return i;
        }
        return 0;
    }
    ...
    private static void genWalls(World world, int x, int y, int z, int color) {
        for (int i = -2; i <= 2; ++i) {
            for (int j = -2; j <= 2; ++j) {
                int k = y + 2;
                do {
                    if (Math.abs(i) == 2 && Math.abs(j) == 2 || world.getBlock(x + i, k, z + j) == Blocks.clay) continue;
    ...
    private static void setDoorAndInteriors(World world, Random rand, int x, int y, int z, int color, ItemStack flower) {
        ForgeDirection doorDir = ForgeDirection.getOrientation((int)(rand.nextInt(4) + 2));
        ...
        for (int i = 2; i < 6; ++i) {
            if (i == doorDir.ordinal()) continue;
            ForgeDirection windowDir = ForgeDirection.getOrientation((int)i);
        ...
        WorldGenClayHut.insertChestLoot(world, rand, x + torchDir.offsetX + torchDir.offsetZ, y + 1, z + torchDir.offsetZ + torchDir.offsetX);
        world.setBlock(x + torchDir.offsetX - torchDir.offsetZ, y + 1, z + torchDir.offsetZ - torchDir.offsetX, Blocks.flower_pot, 0, 3);
        WorldGenClayHut.insertFlower(world, x + torchDir.offsetX - torchDir.offsetZ, y + 1, z + torchDir.offsetZ - torchDir.offsetX, flower);
    }

    private static void placeSoldiers(World world, Random rand, int x, int y, int z, String team) {
        boolean zombified;
        int soldiers = rand.nextInt(2) + 2;
        boolean bl = zombified = ModConfig.clayHutZombieChance > 0 && rand.nextInt(ModConfig.clayHutZombieChance) == 0;
        if (zombified) {
            world.setBlock(x, y + 4, z, Blocks.redstone_torch, 0, 3);
        }
        for (int i = 0; i < soldiers; ++i) {
            double sldX = (float)x + rand.nextFloat();
            double sldY = (double)y + 1.5;
            double sldZ = (float)z + rand.nextFloat();
            ...
            if (zombified) {
                jack.addUpgrade(SoldierUpgrades.getUpgrade("enderpearl"));
            } else {
                jack.addUpgrade(SoldierUpgrades.getUpgrade("wheat"));
            }
    ...
    private static void insertChestLoot(World world, Random rand, int chestX, int chestY, int chestZ) {
        ...
        if (chest != null) {
            WeightedRandomChestContent.generateChestContents(rand, ChestGenHooks.getItems((String)CHEST_CONTENT, (Random)rand), chest, ChestGenHooks.getCount((String)CHEST_CONTENT, (Random)rand));
        }
    }
    ...
    static {
        ChestGenHooks.getInfo((String)CHEST_CONTENT).setMin(4);
        ChestGenHooks.getInfo((String)CHEST_CONTENT).setMax(8);
        ChestGenHooks.addItem((String)CHEST_CONTENT, (WeightedRandomChestContent)new WeightedRandomChestContent(Items.clay_ball, 0, 8, 32, 15));
        ChestGenHooks.addItem((String)CHEST_CONTENT, (WeightedRandomChestContent)new WeightedRandomChestContent(Item.getItemFromBlock(Blocks.clay), 0, 1, 2, 15));
        ChestGenHooks.addItem((String)CHEST_CONTENT, (WeightedRandomChestContent)new WeightedRandomChestContent(RegistryItems.disruptor, 0, 1, 1, 10));
        ChestGenHooks.addItem((String)CHEST_CONTENT, (WeightedRandomChestContent)new WeightedRandomChestContent(RegistryItems.dollBrick, 0, 1, 2, 10));
        ChestGenHooks.addItem((String)CHEST_CONTENT, (WeightedRandomChestContent)new WeightedSoldierDoll(1, 6, 10));
        ChestGenHooks.addItem((String)CHEST_CONTENT, (WeightedRandomChestContent)new WeightedRandomChestContent(RegistryItems.dollHorseMount, 0, 1, 8, 5));
        ChestGenHooks.addItem((String)CHEST_CONTENT, (WeightedRandomChestContent)new WeightedRandomChestContent(RegistryItems.dollTurtleMount, 0, 1, 8, 5));
        ChestGenHooks.addItem((String)CHEST_CONTENT, (WeightedRandomChestContent)new WeightedRandomChestContent(RegistryItems.dollBunnyMount, 0, 1, 8, 5));
        ChestGenHooks.addItem((String)CHEST_CONTENT, (WeightedRandomChestContent)new WeightedRandomChestContent(Item.getItemFromBlock(RegistryBlocks.clayNexus), 0, 1, 1, 1));
        POSSIBLE_TEAMS = ImmutableList.of(Triplet.with("red", 14, new ItemStack(Blocks.red_flower, 1, 0)), Triplet.with("yellow", 4, new ItemStack(Blocks.yellow_flower, 1, 0)), Triplet.with("green", 13, new ItemStack(Blocks.cactus, 1, 0)), Triplet.with("blue", 11, new ItemStack(Blocks.red_flower, 1, 1)), Triplet.with("pink", 6, new ItemStack(Blocks.red_flower, 1, 7)), Triplet.with("purple", 10, new ItemStack(Blocks.red_flower, 1, 2)));
    }
