...
implements IDisruptable {
    ...
    public IIcon getIconIndex(ItemStack stack) {
        return ItemClayManDoll.getTeam(stack).getIconInstance();
    }
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int blockX, int blockY, int blockZ, int side, float offX, float offY, float offZ) {
        ...
        for (int i = 0; i < maxSpawns; ++i) {
            EntityClayMan dan = ItemClayManDoll.spawnClayMan(world, ItemClayManDoll.getTeam(stack).getTeamName(), (double)blockX + 0.5, (double)blockY + entityOffY, (double)blockZ + 0.5);
    ...
    public String getUnlocalizedName(ItemStack par1ItemStack) {
        return super.getUnlocalizedName(par1ItemStack) + '.' + ItemClayManDoll.getTeam(par1ItemStack).getTeamName().toLowerCase();
    }

    @Override
    public int getColorFromItemStack(ItemStack stack, int pass) {
        return ItemClayManDoll.getTeam(stack).getIconColor();
    }
    ...
    public void getSubItems(Item itemInst, CreativeTabs creativeTab, List stacks) {
        for (String team : ClaymanTeam.getTeamNamesForDolls()) {
            ...
            ItemClayManDoll.setTeamForItem(team, stack);
            stacks.add(stack);
        }
    }
