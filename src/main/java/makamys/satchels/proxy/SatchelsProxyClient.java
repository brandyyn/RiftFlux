...
public class SatchelsProxyClient extends SatchelsProxyCommon {
    ...
    public void postInit() {
        super.postInit();
        if(ConfigSatchels.compatTechguns && Loader.isModLoaded("Techguns")) {
    ...
    public void onClientTick(ClientTickEvent event) {
        ...
        if(event.phase == TickEvent.Phase.END) {
            EntityPlayer player = Minecraft.getMinecraft().thePlayer;
            if (player != null && player.inventoryContainer instanceof ContainerSatchels) {
                EntityPropertiesSatchels props = EntityPropertiesSatchels.fromPlayer(player);
                if (props != null && props.refreshEquipmentCache()) {
