...
in vec4 v_Color;
in vec2 v_TexCoord;
in vec3 v_RiftFluxCameraRelativePosition;
...
uniform float u_RiftFluxCutawayRadius;
...
void main() {
    if (u_RiftFluxCutawayRadius > 0.0
            && length(v_RiftFluxCameraRelativePosition) < u_RiftFluxCutawayRadius) {
        discard;
    }
